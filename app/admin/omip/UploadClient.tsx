...
export default function UploadClient({ adminKey }: { adminKey: string }) {
  const [stats, setStats] = useState<any>(null);
  const [msg, setMsg] = useState("");

  useEffect(() => {
    if (!adminKey) return;
    fetch(`/api/admin/omip-upload?key=${encodeURIComponent(adminKey)}`)
      .then((r) => r.json())
      .then(setStats)
      .catch(() => {});
  }, [adminKey]);
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const r = await fetch(
      `/api/admin/omip-upload?key=${encodeURIComponent(adminKey)}`,
      ...
      <div className="rounded-xl border p-4 bg-white">
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div className="text-xs text-slate-600">
            ...
            (opcional <code>source</code>). Exemplos de <code>month</code>:{" "}
            <code>2025-11</code>, <code>2025-11-01</code>, <code>11/2025</code>.
          </div>
